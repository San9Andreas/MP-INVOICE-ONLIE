...
export default function Navbar({ currentPage, onNavigate }: NavbarProps) {
  ...
  const links: { page: Page; label: string; icon: React.ReactNode }[] = [
    { page: 'dashboard', label: 'ဒက်ရှ်ဘုတ်', icon: <LayoutDashboard className="w-4 h-4" /> },
    { page: 'financial', label: 'ငွေကြေးစီမံ', icon: <BarChart3 className="w-4 h-4" /> },
    { page: 'create', label: 'ပြေစာအသစ်', icon: <FilePlus className="w-4 h-4" /> },
    { page: 'history', label: 'မှတ်တမ်း', icon: <Clock className="w-4 h-4" /> },
  ];
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => onNavigate('dashboard')}>
            ...
            <div className="hidden sm:block">
              <span className="text-lg font-bold text-slate-800">ပြေစာ စနစ်</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <div
              ...
              title={
                storageMode === 'firestore' && firestoreConnected
                  ? 'Firebase Firestore နှင့် ချိတ်ဆက်ပြီး — အချက်အလက်များ အချိန်နှင့်တစ်ပြေးညီ sync ဖြစ်သည်'
                  : storageMode === 'firestore'
                  ? 'Firestore ချိတ်ဆက်မှု ပြတ်တောက်သည် — ပြန်ကြိုးစားနေသည်...'
                  : 'Local storage သုံးနေသည် — Cloud sync ဖွင့်ရန် Firebase config ထည့်ပါ'
              }
            >
              {storageMode === 'firestore' && firestoreConnected ? (
                <>
                ...
                <>
                  ...
                  <span className="hidden lg:inline">ပြန်ချိတ်ဆက်နေသည်</span>
                </>
              ) : (
                <>
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-slate-200 bg-white px-4 py-3 space-y-1">
          ...
          <div className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-medium mb-2 ${
            ...
            {storageMode === 'firestore' && firestoreConnected ? (
              <><Cloud className="w-3.5 h-3.5" /><Wifi className="w-3.5 h-3.5" /> Firestore ချိတ်ဆက်ပြီး</>
            ) : (
              <><HardDrive className="w-3.5 h-3.5" /> Local Storage သုံးနေသည်</>
            )}
          </div>
