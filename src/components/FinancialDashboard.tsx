...
export default function FinancialDashboard({ onBack }: Props) {
  ...
  const metrics = useMemo(() => {
    ...
    // Previous period comparison (rough)
    const prevPeriodInvoices = invoices.filter((inv) => {
      const filtered = filteredInvoices.find((f) => f.id === inv.id);
      return !filtered;
    });
  ...
  // ── Monthly Sales Data (for bar & line charts) ─────────────
  ...
  const exportCSV = (data: Invoice[], filename: string) => {
    const headers = [
      'Invoice Number', 'Date', 'Due Date', 'Status', 'Client Name', 'Client Company',
      'Client Email', 'Currency', 'Subtotal', 'Tax Rate (%)', 'Tax Amount',
      'Discount Rate (%)', 'Discount Amount', 'Total', 'Paid Amount', 'Balance Due',
  ...
  const exportSummaryCSV = () => {
    ...
    const rows = [
      ['Report Period', dateFilter === 'custom' ? `${customStart} to ${customEnd}` : dateFilter.replace(/_/g, ' ')],
      ['Total Revenue', metrics.totalRevenue.toFixed(2)],
      ['Total Deposits Collected', metrics.totalDeposits.toFixed(2)],
      ['Total Balance Outstanding', metrics.totalOutstanding.toFixed(2)],
      ['Number of Orders', metrics.orderCount.toString()],
      ['Average Order Value', metrics.avgOrderValue.toFixed(2)],
      ['Collection Rate', `${metrics.collectionRate.toFixed(1)}%`],
      ['', ''],
      ['--- Monthly Breakdown ---', ''],
      ['Month', 'Revenue,Deposits,Outstanding,Orders'],
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 print:p-0">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 ml-8 lg:ml-14">
              Track business performance from your Firestore invoice data
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap w-full lg:w-auto">
                  ...
                  <div className="absolute right-0 top-12 w-80 bg-white border border-slate-200 rounded-2xl shadow-2xl z-40 p-4">
                    ...
                    <div className="space-y-1.5 mb-3">
                      {(['this_month', 'last_3_months', 'last_6_months', 'this_year', 'all_time'] as DateFilter[]).map((f) => (
                        ...
                        >
                          {({ this_month: '📅 This Month', last_3_months: '📊 Last 3 Months', last_6_months: '📈 Last 6 Months', this_year: '🗓️ This Year', all_time: '♾️ All Time', custom: '' } as Record<DateFilter, string>)[f]}
                        </button>
                      ))}
                    </div>
            ...
            <button
              onClick={() => exportCSV(filteredInvoices, `invoices_${dateFilter}_${new Date().toISOString().split('T')[0]}`)}
      ...
      {dateFilter !== 'all_time' && (
        <div className="mb-4 flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1 px-2 py-1 text-xs text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-full transition"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {/* Total Revenue */}
        <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-2xl p-5 text-white shadow-lg shadow-indigo-500/20 relative overflow-hidden">
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-3">
              ...
              {metrics.revenueGrowth !== 0 && (
                <span className={`flex items-center gap-0.5 text-[11px] font-semibold px-2 py-1 rounded-full ${
                  metrics.revenueGrowth > 0 ? 'bg-emerald-400/30 text-emerald-100' : 'bg-red-400/30 text-red-100'
                }`}>
            ...
            <p className="text-indigo-200 text-xs mt-1 font-medium">Total Revenue</p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl p-5 text-white shadow-lg shadow-emerald-500/20 relative overflow-hidden">
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[11px] font-semibold px-2 py-1 rounded-full bg-white/20">
                {metrics.collectionRate.toFixed(0)}%
              </span>
            </div>
            <p className="text-2xl sm:text-3xl font-bold">{fmtCurrency(metrics.totalDeposits)}</p>
            <p className="text-emerald-200 text-xs mt-1 font-medium">Deposits Collected</p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-amber-500 to-orange-500 rounded-2xl p-5 text-white shadow-lg shadow-amber-500/20 relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <p className="text-amber-200 text-xs mt-1 font-medium">Balance Outstanding</p>
          </div>
        </div>

        {/* Orders */}
        <div className="bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl p-5 text-white shadow-lg shadow-purple-500/20 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full -translate-y-8 translate-x-8" />
          <div className="relative">
            <div className="flex items-center justify-between mb-3">
              <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
            ...
            <p className="text-purple-200 text-xs mt-1 font-medium">Number of Orders</p>
            <p className="text-white/70 text-[10px] mt-0.5">Avg: {fmtCurrency(metrics.avgOrderValue)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Avg Order Value</p>
          <p className="text-lg font-bold text-slate-800 mt-1">{fmtCurrency(metrics.avgOrderValue)}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Collection Rate</p>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm mb-8 overflow-hidden">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-5 border-b border-slate-100 gap-3">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Revenue, deposits, and outstanding by month</p>
          </div>
          <div className="flex items-center bg-slate-100 rounded-lg p-0.5">
            <button
              onClick={() => setActiveChart('bar')}
              className={`px-3 py-1.5 text-xs font-medium rounded-md transition ${
                activeChart === 'bar' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
              <BarChart3 className="w-3.5 h-3.5 inline mr-1" /> Bar
            </button>
            <button
              onClick={() => setActiveChart('area')}
              className={`px-3 py-1.5 text-xs font-medium rounded-md transition ${
                activeChart === 'area' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
              <Activity className="w-3.5 h-3.5 inline mr-1" /> Area
            </button>
            <button
              onClick={() => setActiveChart('line')}
              className={`px-3 py-1.5 text-xs font-medium rounded-md transition ${
                activeChart === 'line' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
          ...
          {monthlyData.some((m) => m.revenue > 0) ? (
            <ResponsiveContainer width="100%" height={350}>
              {activeChart === 'bar' ? (
                <BarChart data={monthlyData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                  ...
                  <Bar dataKey="deposits" name="Deposits" fill="#10b981" radius={[6, 6, 0, 0]} maxBarSize={40} />
                  <Bar dataKey="outstanding" name="Outstanding" fill="#f59e0b" radius={[6, 6, 0, 0]} maxBarSize={40} />
                </BarChart>
              ) : activeChart === 'area' ? (
                <AreaChart data={monthlyData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                  ...
                  <Area type="monotone" dataKey="deposits" name="Deposits" stroke="#10b981" fill="url(#colorDeposits)" strokeWidth={2} />
                </AreaChart>
              ) : (
                <LineChart data={monthlyData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                  ...
                  <Line type="monotone" dataKey="deposits" name="Deposits" stroke="#10b981" strokeWidth={2.5} dot={{ r: 4, fill: '#10b981' }} activeDot={{ r: 6 }} />
                  <Line type="monotone" dataKey="outstanding" name="Outstanding" stroke="#f59e0b" strokeWidth={2} strokeDasharray="5 5" dot={{ r: 3, fill: '#f59e0b' }} />
                </LineChart>
              )}
            </ResponsiveContainer>
          ) : (
            <div className="h-[350px] flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm font-medium text-slate-500">No data for selected period</p>
                <p className="text-xs text-slate-400 mt-1">Create invoices to see monthly trends</p>
              </div>
            </div>
          )}
        </div>
      ...
      {/* ── Row: Yearly Growth + Status Distribution ───────── */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="p-5 border-b border-slate-100">
            ...
            <p className="text-xs text-slate-400 mt-0.5">Revenue and deposits growth over the years</p>
          </div>
          <div className="p-4 sm:p-6">
            {yearlyData.length > 0 ? (
              <ResponsiveContainer width="100%" height={280}>
                <LineChart data={yearlyData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                  ...
                  <Legend wrapperStyle={{ fontSize: '12px', paddingTop: '12px' }} />
                  <Line
                    type="monotone" dataKey="revenue" name="Revenue" stroke="#6366f1" strokeWidth={3}
                    dot={{ r: 5, fill: '#6366f1', stroke: '#fff', strokeWidth: 2 }}
                    activeDot={{ r: 7, fill: '#6366f1' }}
                  />
                  <Line
                    type="monotone" dataKey="deposits" name="Deposits" stroke="#10b981" strokeWidth={3}
              ...
              <div className="h-[280px] flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-sm font-medium text-slate-500">No yearly data yet</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden mb-8">
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <div>
            <h2 className="text-sm font-bold text-slate-700 uppercase tracking-wider">Top Clients by Revenue</h2>
            <p className="text-xs text-slate-400 mt-0.5">Your highest-value customers</p>
          </div>
        </div>
        {topClients.length > 0 ? (
                  ...
                  <div className="text-right">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] text-emerald-600 font-medium">
                        Paid: {fmtCurrency(client.paid)}
                      </span>
                      <span className="text-[10px] text-slate-400">
                        ({payRate.toFixed(0)}%)
                      </span>
                      <span className="text-[10px] text-slate-400">{client.count} orders</span>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="p-10 text-center">
            <p className="text-sm text-slate-500">No client data for the selected period</p>
          </div>
        )}
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-300 mt-1">
              Download your financial data as CSV files for accounting and record-keeping
            </p>
          </div>
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button
              onClick={() => exportCSV(filteredInvoices, `invoices_detail_${new Date().toISOString().split('T')[0]}`)}
